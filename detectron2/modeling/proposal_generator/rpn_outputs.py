...
from detectron2.structures import Boxes, Instances, pairwise_iou, cat_boxes, ImageListJitInterface, JittableInstances
...
def find_top_rpn_proposals(
    ...
    Args:
        ...
        min_box_side_len (int): minimum proposal box side length in pixels (absolute units
            wrt input images).
...
def rpn_losses(
    gt_objectness_logits: torch.Tensor,
    gt_anchor_deltas: torch.Tensor,
...
class RPNOutputs(object):
    def __init__(
        ...
        self.num_images = len(self.image_sizes)
    ...
    def losses(self):
        ...
        num_anchors_per_map: List[int] = [torch.prod(torch.tensor(x.shape[1:])).item() for x in self.pred_objectness_logits]
        num_anchors_per_image = torch.sum(torch.tensor(num_anchors_per_map))

        # Stack to: (N, num_anchors_per_image)
        gt_objectness_logits = torch.stack(
            [self._resample(label) for label in gt_objectness_logits], dim=0
        )
